...
```mermaid
flowchart LR
    U["用户点击扩展图标"] --> SW["Manifest V3<br/>Service Worker"]
    SW -->|按需注入| P["网页内悬浮面板<br/>Shadow DOM"]
    P -->|当前控件的本地描述信息| M["智能匹配与分组导航"]
    M -->|仅定位| P
    U -->|点击字段按钮| P
    P -->|按字段请求值| SW
    SW --> S[("chrome.storage.local<br/>本地配置")]
    SW -->|只返回所选字段| P
    P -->|写入并触发标准事件| T["当前选中的表单控件"]
    T -. "不会自动提交" .-> U
```
